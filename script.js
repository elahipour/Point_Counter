...
const changeHandler = (e) => {
  ...
  for (let i = 0; i < arrOfStr.length; i++) {
    singlePoint.map((item) => {
      if (arrOfStr[i] === item) singlePointCounter++;
    });
    twoPoint.map((item) => {
      if (arrOfStr[i] === item) twoPointCounter += 2;
    });
    threePoint.map((item) => {
      if (arrOfStr[i] === item) threePointCounter += 3;
    });
  }
  ...
  result.innerHTML = `  تعداد نقطه ها : ${new Number(
    singlePointCounter + twoPointCounter + threePointCounter
